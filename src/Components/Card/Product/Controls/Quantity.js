...
import { increaseItemQuantity, decreaseItemQuantity, changeItemQuantity } from "../../../../actions/productActions";
...
class Quantity extends Component {
    ...
    render() {
        ...
        return (
            ...
            <button disabled={quantity <= 0 ? true : false} onClick={() => decreaseItem(id)}>-</button>
            <input onChange={(e) => changeItemQuantityOnInput(id, e.target.value)} value={quantity} type="text" />
            <button disabled={ quantity >= 100 ? true : false} onClick={() => increaseItem(id)}>+</button>
        </div>
        )
    }
}
